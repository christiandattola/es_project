...
float dpn1() { return ((drnd() * 2.0f) - 1.0f); }
...
float *alloc_1d_dbl(int n) {
  ...
  nuovo = (float *)malloc((unsigned)((unsigned long)n * sizeof(float)));
  if (nuovo == NULL) {
...
float **alloc_2d_dbl(int m, int n) {
  ...
  nuovo = (float **)malloc(((unsigned long)m * sizeof(float *)));
  if (nuovo == NULL) {
...
void bpnn_save(BPNN *net, char *filename) {
  ...
  */
  ...
  memcnt = 0;
  w = net->input_weights;
  mem = (char *)malloc(((unsigned long)((n1 + 1) * (n2 + 1)) * sizeof(float)));
  for (i = 0; i <= n1; i++) {
    for (j = 0; j <= n2; j++) {
      dvalue = w[i][j];
  ...
  fwrite(mem, (unsigned)(sizeof(float)),
         ((unsigned long)((n1 + 1) * (n2 + 1)) * sizeof(float)), pFile);
  free(mem);

  memcnt = 0;
  w = net->hidden_weights;
  mem = (char *)malloc(((unsigned long)((n2 + 1) * (n3 + 1)) * sizeof(float)));
  for (i = 0; i <= n2; i++) {
    for (j = 0; j <= n3; j++) {
      dvalue = w[i][j];
      fastcopy(&mem[memcnt], &dvalue, sizeof(float));
  ...
  fwrite(mem, sizeof(float),
         ((unsigned long)((n2 + 1) * (n3 + 1)) * sizeof(float)), pFile);
  free(mem);
...
BPNN *bpnn_read(char *filename) {
  ...
  nuovo = bpnn_internal_create(n1, n2, n3);
  ...
  memcnt = 0;
  mem = (char *)malloc(((unsigned long)((n1 + 1) * (n2 + 1)) * sizeof(float)));
  read(fd, mem, (unsigned long)((n1 + 1) * (n2 + 1)) * sizeof(float));
  for (i = 0; i <= n1; i++) {
    for (j = 0; j <= n2; j++) {
      fastcopy(&(nuovo->input_weights[i][j]), &mem[memcnt], sizeof(float));
      memcnt += (int)sizeof(float);
    }
  }
  free(mem);
  ...
  memcnt = 0;
  mem = (char *)malloc(((unsigned long)((n2 + 1) * (n3 + 1)) * sizeof(float)));
  read(fd, mem, (unsigned long)((n2 + 1) * (n3 + 1)) * sizeof(float));
  for (i = 0; i <= n2; i++) {
    for (j = 0; j <= n3; j++) {
      fastcopy(&(nuovo->hidden_weights[i][j]), &mem[memcnt], sizeof(float));
      memcnt += (int)sizeof(float);
    }
  }
  free(mem);
  ...
  bpnn_zero_weights(nuovo->input_prev_weights, n1, n2);
  bpnn_zero_weights(nuovo->hidden_prev_weights, n2, n3);
